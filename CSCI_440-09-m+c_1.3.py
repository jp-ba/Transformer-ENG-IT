#filename: CSCI_440-09-m+c_1.0.py
...
# Reshape the tensor into a 3x4 matrix using view()
viewed_tensor = original_tensor.view(3, 4)
# Shape is (3, 4)
print(f"Viewed shape (3, 4): {viewed_tensor.shape}")
print(viewed_tensor)

# Use -1 to infer one dimension
inferred_view = original_tensor.view(2, -1)
# Shape is (2, 6) -> 12 / 2 = 6
print(f"Inferred shape (2, -1): {inferred_view.shape}")
print(inferred_view)

# Modifying the view changes the original tensor
viewed_tensor[0, 0] = 99
print(f"Original tensor after modification: {original_tensor}") 


#2) .transpose (numpy, not pytorch)
#https://numpy.org/doc/2.2/reference/generated/numpy.transpose.html 

#query = query.view(query.shape[0], query.shape[1], self.h, self.d_k).transpose(1, 2)
#The key matrix is transposed across its last two dimensions to align the dimensions for the matrix multiplication with the query matrix. 
#The result of query @ key.transpose(-2, -1) is a matrix where each entry (i, j) represents the dot product similarity score between the i-th query vector and the j-th key vector.
...
#3) pytorch: dot product vs. elementwise multiplication
import torch
a = torch.tensor([[1,2],
                  [3,4]])
b = torch.tensor([[5,6],
...
#alt example
e = torch.randn(2, 3) #2 rows, 3 cols
f = torch.randn(3, 4) #3 rows, 4 cols
g=e@f
g #2 rows, 4 cols
print(g.dtype) #torch.float32

#4) numpy: practical: pe: Array Slice & Dice

#The NumPy array syntax eg [:, 0::2] is used for slicing a multi-dimensional array to select all elements in the first dimension (rows) and skip every second element in the second dimension (columns), starting from the first column (index 0). 
#The general syntax for slicing a NumPy array is x[start:stop:step] for each dimension. When applied in a multi-dimensional context (e.g., [:, :]), a comma separates the dimensions. 
#The expression can be broken down into two parts:
#: operator (before the comma): This selects all elements along the first dimension (rows). The default values are start=0, stop=size of dimension, and step=1, so leaving them all as a colon means "select all rows".
#eg [:, 0::2], recall x[start:stop:step]
#0::2 (after the comma): This selects elements along the second dimension (columns) with specific slicing parameters:
#0: The start index is 0 (the first column).
#: The stop index is the end of the dimension (default).
#2: The step size is 2, meaning it selects every second column. 
...
arr.shape # (4,5)
# The shape of arr is (4, 5)
arr[:, 0::2]
#all rows
#start: col 0
#end: col n (last column, in this case col 5)
#step: 2 (2nd col, 4th col)
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def attention(query, key, value, mask, dropout: nn.Dropout):
        ...
        # @ operator: to compute a dot product between two tensors in pytorch
        # * operator: to complete elementwise multiplication
        #transpose (-2, -1) means take transpose of the last 2 dimensions. 
        #the 2nd to last dim: seq_len, the 1st to last dim: d_k. 
        #the last 2 dimensions: seq_len and then d_k now transposes to d_k then seq_len
        #then we divide by math.sqrt(d_k)
