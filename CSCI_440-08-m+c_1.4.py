#filename: CSCI_440-08-m+c_1.0.py
...
m = nn.Linear(20, 30)
input = torch.randn(128, 20)    #matrix
output = m(input)
print(output.size())


#II. Code
#practical: pe: Array Slice and Dice

#The NumPy array syntax [:, 0::2] is used for slicing a multi-dimensional array to select all elements in the first dimension (rows) and every second element in the second dimension (columns), starting from the first column (index 0). 
...
#: (before the comma): This selects all elements along the first dimension (rows). The default values are start=0, stop=size of dimension, and step=1, so leaving them all as a colon means "select all rows".
